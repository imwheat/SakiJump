...
namespace KFrame.UI
{
    [AddComponentMenu("KUI/ToggleGroup", 31)]
    public class KToggleGroup : UIBehaviour

    {
        ...
        protected List<KToggle> m_Toggles = new List<KToggle>();
        ...
        private void ValidateToggleIsInGroup(KToggle toggle)
        {
        ...
        public void NotifyToggleOn(KToggle toggle, bool sendCallback = true)
        {
        ...
        public void EnsureValidState()
        {
            ...
            IEnumerable<KToggle> activeToggles = ActiveToggles();

            if (activeToggles.Count() > 1)
            {
                KToggle firstActive = GetFirstActiveToggle();

                foreach (KToggle toggle in activeToggles)
                {
        ...
        /// </remarks>
        public KToggle GetFirstActiveToggle()
        {
            IEnumerable<KToggle> activeToggles = ActiveToggles();
